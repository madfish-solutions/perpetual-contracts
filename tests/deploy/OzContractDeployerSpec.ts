...
describe("OzContractDeployer Spec", () => {
  ...
  const contractNameV1 =
    "src/mock/UpgradableContractV1.sol:UpgradableContractV1";
  const contractNameV2 =
    "src/mock/UpgradableContractV2.sol:UpgradableContractV2";
  ...
  it("force error, initialization is included in ozContractDeployer.deploy()", async () => {
    ...
    const v1Impl = (await ethers.getContractAt(
      contractNameV1,
      v1ImplAddr,
    )) as UpgradableContractV1;
    await expectRevert(
  ...
  describe("upgrade to v2", () => {
    ...
    it("force error, initialization is included in ozContractDeployer.upgrade()", async () => {
      ...
      const v2Impl = (await ethers.getContractAt(
        contractNameV2,
        v2ImplAddr,
      )) as UpgradableContractV2;
      await expectRevert(
  ...
  describe("prepareUpgrade to v2", () => {
    ...
    beforeEach(async () => {
      v2ImplAddr = await ozContractDeployer.prepareUpgrade(
        proxyAddr,
        contractNameV2,
        [],
      );
  ...
  describe("transferProxyAdminOwnership to others", () => {
    it("can't transfer to empty address", async () => {
      await expect(
        OzContractDeployer.transferProxyAdminOwnership(
    ...
    it("can't transfer and upgrade once transfer admin to others, but can deploy new and prepareUpgrade", async () => {
      ...
      await expect(
        OzContractDeployer.transferProxyAdminOwnership(wallet.address),
      ).to.be.reverted;
      await expect(ozContractDeployer.upgrade(proxyAddr, contractNameV2, [])).to
        .be.reverted;
      await upgrades.prepareUpgrade(v1.address, factoryV2);
      const newProxy = await ozContractDeployer.deploy(contractNameV2, []);
      await expect(ozContractDeployer.upgrade(newProxy, contractNameV1, [])).to
        .be.reverted;
    });
