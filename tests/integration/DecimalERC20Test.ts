...
const ERC20MinimalFake = artifacts.require(
  "ERC20MinimalFake",
) as ERC20MinimalFakeContract;
const DecimalERC20Fake = artifacts.require(
  "DecimalERC20Fake",
) as DecimalERC20FakeContract;
const TokenMock = artifacts.require("TokenMock") as TokenMockContract;

describe("DecimalERC20", () => {
  let decimalErc20: DecimalERC20FakeInstance;
  let erc20: ERC20FakeInstance;
  let tether: TokenMockInstance;
  let admin: string;
  let alice: string;
  let bob: string;
  let decimal: BN;
  ...
  describe("decimal = 8", () => {
    ...
    it("approve", async () => {
      ...
      expect(await erc20.allowance(decimalErc20.address, alice)).eq(
        new BN(5).mul(digit),
      );
    });
    ...
    it("allowance", async () => {
      ...
      expect(await decimalErc20.allowance(erc20.address, admin, alice)).eq(
        toFullDigit(5),
      );
    });
    ...
    it("balanceOf", async () => {
      ...
      await erc20.transfer(decimalErc20.address, fiveInEightDigit);
      expect(
        await decimalErc20.balanceOf(erc20.address, decimalErc20.address),
      ).eq(toFullDigit(5));
    });

    it("transferFrom", async () => {
      await erc20.approve(decimalErc20.address, new BN(5).mul(digit));
      await decimalErc20.transferFrom(
        erc20.address,
        admin,
        alice,
        toDecimal(5),
  ...
  describe("non-standard ERC20 (tether)", () => {
    beforeEach(async () => {
      tether = await TokenMock.new(toFullDigit(100), "Tether", "USDT", 6);
    ...
    it("approve", async () => {
      ...
      expect(await tether.allowance(decimalErc20.address, alice)).eq(
        new BN(5).mul(digit),
      );
    });
    ...
    it("allowance", async () => {
      ...
      expect(await decimalErc20.allowance(tether.address, admin, alice)).eq(
        toFullDigit(5),
      );
    });
    ...
    it("balanceOf", async () => {
      ...
      await tether.transfer(decimalErc20.address, fiveInEightDigit);
      expect(
        await decimalErc20.balanceOf(tether.address, decimalErc20.address),
      ).eq(toFullDigit(5));
    });
    ...
    it("transferFrom with decimals", async () => {
      ...
      await decimalErc20.transferFrom(tether.address, bob, alice, {
        d: "4999999999999",
      });
    ...
    it("transfer with decimals", async () => {
      ...
      await decimalErc20.transfer(tether.address, alice, {
        d: "4999999999999",
      });
    ...
    describe("with fee (same as deflationary token)", () => {
      beforeEach(async () => {
        tether = await TokenMock.new(toFullDigit(1000), "Tether", "USDT", 6);
        decimal = await tether.decimals();
        digit = new BN(10).pow(decimal);
  ...
  describe("approve", () => {
    ...
    it("re-approve ERC20: force error, approve again without resetting to 0", async () => {
      await expectRevert(
        tether.approve(alice, toFullDigit(50)),
        "Transaction reverted without a reason",
      );
    });

    it("DecimalERC20: approve", async () => {
      const r = await decimalErc20.approve(
        tether.address,
        alice,
        toDecimal(50),
      );
