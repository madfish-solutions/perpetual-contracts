...
describe("ClearingHouse Test", () => {
  ...
  async function endEpoch(): Promise<void> {
    await forwardBlockTimestamp(
      (await supplySchedule.mintDuration()).toNumber(),
    );
  ...
  async function approve(
    ...
    await quoteToken.approve(
      spender,
      toFullDigit(amount, +(await quoteToken.decimals())),
      { from: account },
    );
  }
  ...
  describe("getPersonalPositionWithFundingPayment", () => {
    it("return 0 margin when alice's position is underwater", async () => {
      ...
      expect(
        await clearingHouse.getLatestCumulativePremiumFraction(amm.address),
      ).eq(toFullDigit(-0.5));
  ...
  describe("openInterestNotional", () => {
    ...
    it("increase when increase position", async () => {
      ...
      expect(await clearingHouse.openInterestNotionalMap(amm.address)).eq(
        toFullDigitStr(600),
      );
    });
    ...
    it("reduce when close position", async () => {
      ...
      const openInterestNotional = await clearingHouse.openInterestNotionalMap(
        amm.address,
      );
  ...
  describe("payFunding: when alice.size = 37.5 & bob.size = -187.5", () => {
    beforeEach(async () => {
      ...
      const clearingHouseBaseTokenBalance = await quoteToken.balanceOf(
        clearingHouse.address,
      );
      // 300 (alice's margin) + 1200 (bob' margin) = 1500
      expect(clearingHouseBaseTokenBalance).eq(
        toFullDigit(1500, +(await quoteToken.decimals())),
      );
    });
    ...
    it("will generate loss for amm when funding rate is positive and amm hold more long position", async () => {
      ...
      const clearingHouseQuoteTokenBalance = await quoteToken.balanceOf(
        clearingHouse.address,
      );
      expect(clearingHouseQuoteTokenBalance).to.eq(
        toFullDigit(1501.5, +(await quoteToken.decimals())),
      );
      const insuranceFundBaseToken = await quoteToken.balanceOf(
        insuranceFund.address,
      );
      expect(insuranceFundBaseToken).to.eq(
        toFullDigit(4998.5, +(await quoteToken.decimals())),
      );
    });
    ...
    it("will keep generating the same loss for amm when funding rate is positive and amm hold more long position", async () => {
      ...
      expect(insuranceFundBaseToken).to.eq(
        toFullDigit(4997, +(await quoteToken.decimals())),
      );
    });
    ...
    it("has huge funding payment profit that doesn't need margin anymore", async () => {
      ...
      await clearingHouse.removeMargin(amm.address, toDecimal(400), {
        from: alice,
      });
    ...
    it("has huge funding payment loss that the margin become 0 with bad debt of long position", async () => {
      ...
      const receipt = await clearingHouse.liquidate(amm.address, bob, {
        from: alice,
      });
    ...
    it("will change nothing if the funding rate is 0", async () => {
      ...
      // clearingHouse: 1500
      // insuranceFund: 5000
      const clearingHouseBaseToken = await quoteToken.balanceOf(
        clearingHouse.address,
      );
      expect(clearingHouseBaseToken).to.eq(
        toFullDigit(1500, +(await quoteToken.decimals())),
      );
      const insuranceFundBaseToken = await quoteToken.balanceOf(
        insuranceFund.address,
  ...
  describe("getMarginRatio", () => {
    it("get margin ratio", async () => {
      ...
      const marginRatio = await clearingHouse.getMarginRatio(
        amm.address,
        alice,
      );
    ...
    describe("verify margin ratio when there is funding payment", () => {
      ...
      it("with pnl and funding rate is positive", async () => {
        ...
        const bobMarginRatio = await clearingHouseViewer.getMarginRatio(
          amm.address,
          bob,
        );
  ...
  describe("liquidate", () => {
    ...
    it("partially liquidate a long position", async () => {
      ...
      await clearingHouse.setMaintenanceMarginRatio(toDecimal(0.1), {
        from: admin,
      });
      ...
      expect(
        (await clearingHouse.getPosition(amm.address, alice)).margin,
      ).to.eq("19274981656679729691");
      expect(await clearingHouse.getMarginRatio(amm.address, alice)).to.eq(
        "43713253015241334",
      );
    ...
    it("partially liquidate a long position with quoteAssetAmountLimit", async () => {
      ...
      await expectRevert(
        clearingHouse.liquidateWithSlippage(
          amm.address,
          alice,
          toDecimal(273.85),
          { from: carol },
      ...
      await clearingHouse.liquidateWithSlippage(
        amm.address,
        alice,
        toDecimal(273.8),
        { from: carol },
    ...
    it("force error, position not liquidatable due to TWAP over maintenance margin", async () => {
      ...
      await forwardBlockTimestamp(15);
      const positionBefore = await clearingHouse.getPosition(
        amm.address,
        alice,
      );
      ...
      ).to.eq(new BN("-15384615384615384623"));
      expect(
        await clearingHouseViewer.getUnrealizedPnl(
          amm.address,
          alice,
      ...
      ).to.eq(new BN("-9386059949440231138"));
    ...
    it("force error, position not liquidatable due to SPOT price over maintenance margin", async () => {
      ...
      ).to.eq(new BN("-16388888888888888891"));
    ...
    describe("fluctuation check when liquidating", () => {
      ...
      it("partially liquidate one position within the fluctuation limit", async () => {
        ...
        expect(
          parseFloat(baseAssetReserve.toString().substr(0, 6)) / 10000,
        ).to.eq(92.803);
        expect(
          parseFloat(quoteAssetReserve.toString().substr(0, 6)) / 100,
        ).to.eq(1077.55);
      });

      it("partially liquidate two positions within the fluctuation limit", async () => {
        await amm.setFluctuationLimitRatio(toDecimal(0.041));
        traderWallet1 = await TraderWallet.new(
          clearingHouse.address,
          quoteToken.address,
        );
        ...
        await clearingHouse.setMaintenanceMarginRatio(toDecimal(0.199), {
          from: admin,
        });
      ...
      it("partially liquidate three positions within the fluctuation limit", async () => {
        ...
        await clearingHouse.setBackstopLiquidityProvider(
      ...
      it("partially liquidate one position with the price impact exceeding the fluctuation limit ", async () => {
        ...
        await clearingHouse.setMaintenanceMarginRatio(toDecimal(0), {
          from: admin,
        });
    ...
    describe("liquidator front run hack", () => {
      ...
      it("liquidator can't open and liquidate position in the same block, even from different msg.sender", async () => {
        ...
        traderWallet2 = await TraderWallet.new(
          clearingHouse.address,
          quoteToken.address,
        );
        await clearingHouse.setBackstopLiquidityProvider(
        ...
        await syncAmmPriceToOracle();
        await traderWallet2.liquidate(amm.address, alice, toDecimal(0), {
          from: bob,
        });
        await expectRevert(
  ...
  describe("close position limit", () => {
    ...
    describe("slippage limit", () => {
      ...
      it("closePosition, originally long, (amount should pay = 118.03279) at the limit of min quote amount = 118", async () => {
        ...
        await forwardBlockTimestamp(15);
        await clearingHouse.closePosition(amm.address, toDecimal(118), {
          from: bob,
        });
      ...
      it("closePosition, originally short, (amount should pay = 78.048) at the limit of max quote amount = 79", async () => {
        ...
        // AMM after: 878.0487804877 : 113.8888888889
        await forwardBlockTimestamp(15);
        await clearingHouse.closePosition(amm.address, toDecimal(79), {
          from: bob,
        });
        ...
        const baseAssetReserve = await amm.baseAssetReserve();
        expect(
          parseFloat(quoteAssetReserve.toString().substr(0, 6)) / 1000,
        ).to.eq(878.048);
        expect(
          parseFloat(baseAssetReserve.toString().substr(0, 6)) / 1000,
        ).to.eq(113.888);
      });
      ...
      it("force error, closePosition, originally long, less than min quote amount = 119", async () => {
        ...
        await forwardBlockTimestamp(15);
        await expectRevert(
          clearingHouse.closePosition(amm.address, toDecimal(119), {
            from: bob,
          }),
          "Less than minimal quote token",
        );
      ...
      it("force error, closePosition, originally short, more than max quote amount = 78", async () => {
        ...
        await forwardBlockTimestamp(15);
        await expectRevert(
          clearingHouse.closePosition(amm.address, toDecimal(78), {
            from: bob,
          }),
          "More than maximal quote token",
        );
  ...
  describe("pausable functions", () => {
    it("pause by admin", async () => {
      ...
      await expectRevert(
        clearingHouse.addMargin(amm.address, toDecimal(1)),
        error,
      );
      await expectRevert(
        clearingHouse.removeMargin(amm.address, toDecimal(1)),
        error,
      );
      await expectRevert(
        clearingHouse.closePosition(amm.address, toDecimal(0)),
    ...
    it("can't pause by non-admin", async () => {
      await expectRevert(
        clearingHouse.pause({ from: alice }),
        "PerpFiOwnableUpgrade: caller is not the owner",
      );
    });

    it("pause then unpause by admin", async () => {
      await quoteToken.approve(clearingHouse.address, toFullDigit(2), {
        from: alice,
      });
      await clearingHouse.pause();
  ...
  describe("restriction mode", () => {
    ...
    it("trigger restriction mode", async () => {
      ...
      const blockNumber = new BigNumber(
        await clearingHouse.mock_getCurrentBlockNumber(),
      );
      expect(await clearingHouse.isInRestrictMode(amm.address, blockNumber)).eq(
        true,
      );
      expect(
        await clearingHouse.isInRestrictMode(amm.address, blockNumber.subn(1)),
      ).eq(false);
    });
  ...
  describe("backstop LP setter", async () => {
    it("set backstop LP by owner", async () => {
      expect(await clearingHouse.backstopLiquidityProviderMap(alice)).to.be
        .false;
      await clearingHouse.setBackstopLiquidityProvider(alice, true);
      expect(await clearingHouse.backstopLiquidityProviderMap(alice)).to.be
        .true;
